...
export default function renderProductCard(product: any): string {
  const stars =
    ...
    <div class="max-w-6xl m-6">
        <div class="bg-white rounded-lg shadow p-4 relative flex flex-col h-full">
            ...
            <div class="mt-auto flex space-x-4 pt-4">
                <button onclick="window.location.hash='#products/${product.id}'" class="px-4 py-2 border border-indigo-500 text-indigo-500 rounded-lg hover:bg-indigo-50 transition-colors duration-300">
                    Detail
                </button>
